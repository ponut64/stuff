...
///Sample clipping is due to using software timers for sound playback instead of a hardware timer.
///Though this system almost exactly works, because the CPUs are not infinitely fast,
///it will just barely miss the correct time to play back the next sound.
...
void	pop_load_gvp(void(*game_code)(void)){
	Sint32			rdsize;
	Sint32			nsct_f;
	Sint32			fsize;
	Sint32			stat;
...
//Open GFS
	gfsx = GFS_Open((Sint32)activeGVP->filename);
//Get sectors
...
for( ; fetch_timer >= (mcpy_factor) && fetch_timer <= musicTimer && mrd_pos == buf_pos ; ){
	workAddress = (activeGVP->useHiMem == true) ? active_HWRAM_ptr : active_LWRAM_ptr;
...
do{
	...
	activeGVP->tmodel->size = (unsigned int)workAddress;
	workAddress = loadPDATA((workAddress), activeGVP->tmodel, &bufModelX);
	activeGVP->tmodel->size = (unsigned int)workAddress - activeGVP->tmodel->size;


	setTextures(activeGVP->tmodel, numTex, &activeGVP->tmodel->numTexture);
    workAddress = loadAnimations(workAddress, activeGVP->tmodel, &bufModelX);
	...
	if(activeGVP->useHiMem == true) //High / low work area switch
	{
	...
	// jo_printf(1, 14, "radZ (%i)", bufModelX.radius[Z]);
		...
		rd_frames = 0;
		curRdFrame = 0;
	activeGVP->tmodel->file_done = true;
	activeGVP->active = false;
...
//transfer should be SCU type since it's going from A to B bus.
...
for( ; ; ){
	...
	if(mrd_pos > MUS_BUFCNT-1){
		mrd_pos = 0;
		}
	if((model_requested == true || sound_requested == true ||
		map_requested == true || tga_requested == true || chg_music == true)
		&& fetch_timer >= mcpy_factor
		&& fetch_timer <= musicTimer 
		&& mrd_pos == buf_pos){
		break;
	}
}
